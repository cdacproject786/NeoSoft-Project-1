package com.admin.controller;

import com.admin.IService.ItransportService;
import com.admin.exterservice.TransporterServive;
import com.admin.service.TransportService;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.http.ResponseEntity;
import org.springframework.web.bind.annotation.GetMapping;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.RestController;

@RestController
@RequestMapping("/admin")
public class TransportController {


    @Autowired
    private ItransportService transportService;

    @GetMapping("/transporter")
    public ResponseEntity<?> getAllTransporter()
    {
        return transportService.getDetails();
    }

    @GetMapping("/transportOrders")
    public ResponseEntity<?> getAlltransportOrders()
    {
        return transportService.getAllOrders();
    }


}
